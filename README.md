...
A full-stack doctor appointment booking platform with three dedicated portals — **Patient**, **Doctor**, and **Admin** — supporting online consultation booking, doctor management, and integrated Razorpay payments.

**Live Demo:** [https://book-my-doctor-one.vercel.app](https://book-my-doctor-one.vercel.app)
...
| `admin` | Management dashboard | Admin & Doctor |
| `backend` | REST API server | Shared by all three |
...
| Contact | Doctor Profile | Book Appointment |
|---|---|---|
...
- Update personal profile (with image upload via Cloudinary)
...
> **Note:** Since `frontend` and `admin` use Vite, all `VITE_*` variables are baked into the JS bundle at **build time**. When deploying on Vercel, adding or changing these variables requires a fresh deployment (redeploy without build cache) to take effect.
